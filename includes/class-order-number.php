<?php
/*
 *  Thanks to the plugin === WooCommerce Sequential Order Numbers ===
 * https://wordpress.org/plugins/woocommerce-sequential-order-numbers/
 *   
 */

class WC_EZOSC_Seq_Order_Number {
	...
	/** version option name */
	const VERSION_OPTION_NAME = 'woocommerce_ezosc_seq_order_number_db_version';
			...
			if ( '' === $order_number ) {
				...
				for ( $i = 0; $i < 3 && ! $success; $i++ ) {
					...
					$success = $wpdb->query( $query );
                            ...
                            $new_order_number = $this->generate_order_number ( $order_number );
                            if ($new_order_number != ''){
                                add_post_meta( $post_id, '_order_number_new', $new_order_number ); 
                            }
                        }
		}
	}
	...
	public function get_order_number( $order_number, $order ) {
		...
		if ( $order->order_number_new ) {
                    return $order->order_number_new;
	...
	private function install() {
		$installed_version = get_option( WC_EZOSC_Seq_Order_Number::VERSION_OPTION_NAME );
		...
		if ( $installed_version !== WC_EZOSC_Seq_Order_Number::VERSION ) {
			$this->upgrade( $installed_version );

			// new version number
			update_option( WC_EZOSC_Seq_Order_Number::VERSION_OPTION_NAME, WC_EZOSC_Seq_Order_Number::VERSION );
		}
	}
	...
	private function upgrade( $installed_version ) {
        ...
        //////////////////////////////////////////////
        // Addition to the Original Plugin
        //////////////////////////////////////////////
        
        /**
	 * Generate New order number
	 */
        public static function generate_order_number ( $order_number ) {
            $new_order_number = $order_number;
            if ($order_number == '') {$order_number = time();}            
            
            $options = get_option('woo-extras-son');
                        
            //Order Number Start
            $woo_extra_son_ons = $options['woo_extra_son_ons'];
            if ($woo_extra_son_ons > 0){
                $order_number = $order_number + $woo_extra_son_ons;
            }
                
            //Order Number Prefix
            $woo_extra_son_onsu = $options['woo_extra_son_onp'];
            if ($woo_extra_son_onsu != ''){            
                $new_order_number = $woo_extra_son_onsu.$order_number;
            }
